...
export const filterBoatRampsData = (features: Array<Feature>, upperBound: any, lowerBound: any) => {
    return features && features.filter(({ geometry })=> {
      return (
...
export const getBoatRampsFeatureMaterialsCount =  (boatRampMapFeatures: Array<Feature>) => {
  ...
  boatRampMapFeatures && Array.isArray(boatRampMapFeatures) && boatRampMapFeatures.forEach(({ properties }) => {
    const propertiesMaterial: string = properties?.material;
    const propertiesArea: number = properties?.area_;
    
    if(
      propertiesMaterialCount.hasOwnProperty(propertiesMaterial) && 
      propertiesMaterial &&
