...
import {
  ...
  BoatRampsViewPortAction,
  BoatRampsViewPortState,
  Feature,
  FetchBoatRampsDataFilterAction,
} from '../type';
...
const boatRampsMapFeaturesReducer = (
  state = INIT_BOAT_RAMPS_MAP_FEATURES,
  action: BoatRampsMapAction
): BoatRampsMapFeaturesState => {
  switch (action.type) {
...
const boatRampsMapViewPortReducer = (
  state = INIT_BOAT_RAMPS_MAP_VIEWPORT,
  action: BoatRampsViewPortAction
): BoatRampsViewPortState => {
  switch (action.type) {
